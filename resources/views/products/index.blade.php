...
<body class="px-2">
    <section class=" w-full md:w-2/3 lg:w-1/2 mx-auto mt-4 bg-slate-400 p-5 rounded-xl shadow-lg">
        ...
        <a href="{{ url('/products/create') }}" class="bg-indigo-500 py-1 px-2 rounded-lg font-semibold text-white hover:bg-indigo-400 transition-all duration-200">Add Product</a>
        <table class="table-auto w-full mt-4 bg-white shadow-lg roundedxl">
            ...
            <tr class="border-b-2 border-slate-300 text-center">
                ...
                <td class="py-1 flex justify-center items-center px-1 gap-2">
                    <a href="{{ url('/products', $product->id) }}" class="bg-blue-700 py-1 px-2 rounded-lg text-white font-semibold hover:bg-blue-400 transition-all duration-200">View</a>
                    <a href="{{ url('/products/' . $product->id . '/edit') }}" class="bg-yellow-500 py-1 px-2 rounded-lg text-white font-semibold hover:bg-yellow-400 transition-all duration-200">Edit</a>
                    <form action="{{ url('/products', $product->id) }}" method="POST" style="display:inline;">
                        @csrf
                        @method('DELETE')
                        <button type="submit" onclick="return confirm('Are you sure?')" class="bg-red-600 py-1 px-2 rounded-lg text-white font-semibold hover:bg-red-500 transition-all duration-200 cursor-pointer">Delete</button>
                    </form>
                </td>
            </tr>
            @endforeach
